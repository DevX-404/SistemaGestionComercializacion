       ...
       <table class="monster-table">
         ...
         <tbody>
           <tr v-for="c in categorias" :key="c._id" :class="{'row-inactive': !c.estado}">
             ...
             <td class="text-right">
                <button v-if="c.estado" class="btn-action delete" @click="intentarEliminar(c)" title="Dar de baja">
                  🗑️
                </button>
...
.monster-table th { text-align: left; padding: 15px; border-bottom: 1px solid #eee; color: #8898aa; font-size: 0.75rem; text-transform: uppercase; letter-spacing: 1px; background: #f6f9fc; }
.monster-table td { padding: 15px; border-bottom: 1px solid #f5f5f5; vertical-align: middle; }

.fw-bold { font-weight: 700; }
.text-main { color: #32325d; }
.text-muted { color: #8898aa; font-size: 0.85rem; }

.status-badge { padding: 5px 12px; border-radius: 20px; font-size: 0.7rem; font-weight: 800; letter-spacing: 0.5px; }
.status-badge.active { background: #eafaf1; color: #2dce89; }
.status-badge.inactive { background: #fdedec; color: #f5365c; }
.row-inactive { opacity: 0.6; background: #f9f9f9; filter: grayscale(100%); }
.locked-icon { font-size: 1.2rem; opacity: 0.5; }

.btn-primary { background: #5e72e4; color: white; border: none; padding: 10px 20px; border-radius: 5px; font-weight: bold; cursor: pointer; box-shadow: 0 4px 6px rgba(50,50,93,.11); transition: 0.2s; }
.btn-primary:hover { transform: translateY(-1px); box-shadow: 0 7px 14px rgba(50,50,93,.1); }

.btn-action.delete { background: white; color: #f5365c; border: none; cursor: pointer; font-size: 1.1rem; transition: 0.2s; width: 35px; height: 35px; border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
.btn-action.delete:hover { background: #f5365c; color: white; }

/* Modal */
.modal-backdrop { position: fixed; top:0; left:0; width:100%; height:100%; background: rgba(0,0,0,0.5); display: flex; justify-content: center; align-items: center; z-index: 1000; backdrop-filter: blur(3px); }
.modal-card { background: white; border-radius: 10px; box-shadow: 0 15px 35px rgba(50,50,93,.2); overflow: hidden; display: flex; flex-direction: column; }
...
.modal-footer { padding: 20px; border-top: 1px solid #eee; text-align: right; background: #f8f9fe; }
.close-btn { background: none; border: none; font-size: 1.5rem; cursor: pointer; color: #889; }

.form-group { margin-bottom: 20px; }
.form-group label { display: block; margin-bottom: 8px; font-weight: 600; color: #525f7f; font-size: 0.85rem; }
.req { color: #f5365c; }
.input-styled { width: 100%; padding: 12px; border: 1px solid #dee2e6; border-radius: 5px; box-sizing: border-box; transition: 0.2s; color: #32325d; }
.input-styled:focus { border-color: #5e72e4; box-shadow: 0 1px 3px rgba(50,50,93,.15); outline: none; }
...
.argon-toast { background: white; width: 320px; padding: 15px; border-radius: 6px; display: flex; gap: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.15); border-left: 5px solid; animation: slideInRight 0.3s; }
...
.toast-title { margin: 0; font-size: 0.85rem; font-weight: 800; color: #32325d; text-transform: uppercase; }
.toast-msg { margin: 4px 0 0; font-size: 0.85rem; color: #525f7f; line-height: 1.4; }
.toast-close { margin-left: auto; background: none; border: none; font-size: 1.2rem; cursor: pointer; color: #adb5bd; }
.toast-close:hover { color: #525f7f; }

.empty-state { text-align: center; padding: 40px; color: #8898aa; }
